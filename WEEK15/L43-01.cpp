...
public:
    ...
    void insert(int val)
    {
        ...
        while (index > 1) // when index is 1 then we dont need to check of its parent index cause
                          //    we cant go more upward anymore we're already at the right posi
        {
    ...
    int deleteFromHeap()
    {
        ...
        while (index < size)
        {
            ...
            // yha tak ya to largestIndex ,rightIndex hogya hoga ya to left ya same index hi hoga
            // so if no change
            if (index == largestKaIndex)
            {
                break;
            }
            // if largestIndex is chnaged(i.e either its become leftIndex or right)
            else
            {
...
void heapSort(int arr[], int n)
{
    while (n != 1)
    {
        ...
        // shrink array size
        n--;
        // heapify the rest of the array after shrinking
        heapify(arr, n, 1);
    }
}
